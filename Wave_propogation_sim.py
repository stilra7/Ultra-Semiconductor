...
def Ef(E0,E1):
    ...
    for i in range(len(E1[:-2])):
        #loops through and determines where the dielectric is and changes the speed of light accordingly
        if x[i] > -5e-6 and x[i] < 5e-6:
            c = 2.9979e8/1.5
        else:
            c = 2.9979e8
        E2.append((((c**2)*(-E1[i+2]+(16*E1[i+1])-30*E1[i]+16*E1[i-1]-E1[i-2])*(dt**2)/(12*(dx**2)))+2*E1[i])-E0[i])
